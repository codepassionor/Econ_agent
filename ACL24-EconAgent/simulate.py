...
def extract_dialogue_content(dialog_queue):
    all_contents = []
    for agent_dialog in dialog_queue:
        for entry in agent_dialog:
            all_contents.append(entry) 
        # print(agent_dialog)
        # all_contents.append(agent_dialog['content'])
    return all_contents

def exponential_decay_weights(L, lambda_):
    return np.exp(-lambda_ * np.arange(L))

def weighted_history_dialogs(dialog4ref_queue, lambda_):
    weighted_dialogs = []
    L = len(dialog4ref_queue)
    weights = exponential_decay_weights(L, lambda_)
    
    for idx, dialog in enumerate(dialog4ref_queue):
        weight = weights[idx]
        weighted_content = f"Weight: {weight:.2f}\n{extract_dialogue_content(dialog)}"
        
        weighted_dialogs.append(weighted_content)
        if len(weighted_dialogs) > 4:
            weighted_dialogs.pop(0)  
    
    return weighted_dialogs 


def generate_reflection_prompt(dialog4ref_queue, lambda_=0.1):
    weighted_dialogs = weighted_history_dialogs(dialog4ref_queue, lambda_)
    print(f'length of weighted_dialogs is {len(weighted_dialogs)}')
    weighted_history = "\n".join(weighted_dialogs)
    ...
    Besides, Assign an economic sentiment score between -1 (very pessimistic) and +1 (very optimistic) based on the overall market trends. Also, provide a brief explanation for this score.
...
def gpt_actions(env, obs, dialog_queue, dialog4ref_queue, gpt_path, gpt_error, total_cost):
    ...
    for idx in tqdm(range(env.num_agents), desc="Processing Agents"):        
        ...
        obs_prompt = f'''
                        ...
                        Also, provide a confidence score between 0 and 1 indicating how certain you are about this decision given current economic conditions.
                    '''
        # obs_prompt = f'''
        #                 {problem_prompt} {job_prompt} {consumption_prompt} {tax_prompt} {price_prompt}
        #                 Your current savings account balance is ${wealth:.2f}. Interest rates, as set by your bank, stand at {interest_rate*100:.2f}%. 
    ...
    if (env.world.timestep+1)%3 == 0:
        ...
        # Your answer must be less than 300 words! 
        # '''
        reflection_prompt = generate_reflection_prompt(dialog4ref_queue, lambda_=0.1)
...
def main(policy_model='gpt', num_agents=100, episode_length=240, dialog_len=3, beta=0.1, gamma=0.1, h=1, max_price_inflation=0.1, max_wage_inflation=0.05):
    ...
    if policy_model == 'gpt':
        policy_model_save = f'{policy_model}-{dialog_len}-full'
    ...
    for epi in tqdm(range(env.episode_length), desc="Simulating Episodes"):
        if policy_model == 'gpt':
