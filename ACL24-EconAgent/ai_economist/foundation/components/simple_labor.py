...
class SimpleLabor(BaseComponent):
    ...
    def get_agent_history(self, agent_idx):
        return self.agent_history.get(agent_idx, None)
    ...
    def get_investment_environment_factor(self):
        """
        Call OpenAI API to get the investment environment index I_t, in the range of 0-1.
        If the return value is not a valid number, keep retrying until successful.
        The API uses historical macroeconomic data to predict the investment index.
        """
        from openai import OpenAI
        
        base_url = "https://openkey.cloud/v1"
        api_key = "sk-xmY83ICHAbw95y8RBeF0B92143154dF78b0bC762912d78D8"  # Please use your actual API key
        client = OpenAI(base_url=base_url, api_key=api_key)

        total_products = self.world.total_products  # List of total products over time
        price = self.world.price 
        unemployment = self.world.unemployment  
        unemployment_rate_inflation = self.world.unemployment_rate_inflation  
        nominal_gdp = self.world.nominal_gdp 
        real_gdp = self.world.real_gdp 

        dialogs = [
            {"role": "system", "content": f"""
            Please evalute the investment confidence index I_t (range 0 to 1) based on the following macroeconomic factors:
            - Total Products: {total_products}
            - Price Levels: {price}
            - Unemployment Rates: {unemployment}
            - Unemployment Rate Inflation: {unemployment_rate_inflation}
            - Nominal GDP: {nominal_gdp}
            - Real GDP: {real_gdp}
            The output should only be a float number between 0 and 1, representing the investment environment index I_t. Do not always make output zero.
            """}
        ]
        ...
        while True:  
            try:
                ...
                I_t_str = response.choices[0].message.content.strip()

                try:
                    I_t = float(I_t_str)
                    return max(0, min(1, I_t))
                except ValueError:
                    print(f"API returned a non-numeric value: {I_t_str}. Retrying...")
                    continue 
